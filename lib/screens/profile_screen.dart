import 'package:get/get.dart';
import 'package:flutter/material.dart'; // IconData, AlertDialog ve SnackBar için
import 'package:firebase_auth/firebase_auth.dart';
import 'package:cloud_firestore/cloud_firestore.dart';
import '../main.dart'; // Routes sınıfına erişmek için

class ProfilScreen extends GetxController {
  final FirebaseAuth _auth = FirebaseAuth.instance;
  final FirebaseFirestore _firestore = FirebaseFirestore.instance;

  // Kullanıcı verilerini reaktif olarak tutuyoruz
  final Rx<Map<String, dynamic>?> userData = Rx<Map<String, dynamic>?>(null);
  // Yüklenme durumunu reaktif olarak tutuyoruz
  final RxBool isLoading = true.obs;

  // Getter'lar ile daha temiz erişim
  String get firstName => userData.value?['firstName'] ?? '';
  String get lastName => userData.value?['lastName'] ?? '';
  String get title => userData.value?['title'] ?? '';
  String get location => userData.value?['location'] ?? '';
  String get email => userData.value?['email'] ?? '';

  @override
  void onInit() {
    super.onInit();
    // Controller başlatıldığında kullanıcı verilerini yükle
    _loadUserData();
    // Firebase Auth durumu değiştiğinde (örneğin çıkış yapıldığında) dinle
    _auth.authStateChanges().listen((User? user) {
      if (user == null) {
        // Kullanıcı çıkış yaptıysa veya oturumu kapandıysa
        userData.value = null;
        isLoading.value = false;
        Get.offAllNamed(Routes.LOGIN); // Giriş ekranına yönlendir
      } else {
        _loadUserData(); // Kullanıcı oturum açtıysa verileri yeniden yükle
      }
    });
  }

  // Kullanıcı verilerini Firestore'dan yükleme metodu
  Future<void> _loadUserData() async {
    isLoading.value = true;
    final uid = _auth.currentUser?.uid;
    if (uid == null) {
      userData.value = null;
      isLoading.value = false;
      print('ProfilScreen: No user ID found.');
      return;
    }

    try {
      final doc = await _firestore.collection('users').doc(uid).get();
      if (doc.exists) {
        userData.value = doc.data();
        print('ProfilScreen: User data loaded for UID: $uid');
      } else {
        userData.value = null;
        print('ProfilScreen: User data not found for UID: $uid');
      }
    } catch (e) {
      print('ProfilScreen: Error loading user data: $e');
      Get.snackbar(
        'Hata',
        'Kullanıcı bilgileri yüklenirken bir hata oluştu: $e',
        backgroundColor: Colors.red,
        colorText: Colors.white,
        snackPosition: SnackPosition.BOTTOM,
      );
      userData.value = null;
    } finally {
      isLoading.value = false;
    }
  }

  // Çıkış yapma metodu
  Future<void> logout() async {
    // Çıkış onayı için Get.dialog kullanıldı
    final confirm = await Get.dialog<bool>(
      AlertDialog(
        title: const Text('Çıkış Yap'),
        content: const Text(
          'Hesabınızdan çıkış yapmak istediğinize emin misiniz?',
        ),
        actions: [
          TextButton(
            onPressed: () =>
                Get.back(result: false), // Dialog'u kapat ve false döndür
            child: const Text('Vazgeç'),
          ),
          TextButton(
            onPressed: () =>
                Get.back(result: true), // Dialog'u kapat ve true döndür
            child: const Text('Çıkış Yap'),
          ),
        ],
      ),
    );

    if (confirm == true) {
      try {
        await _auth.signOut();
        // authStateChanges listener'ı zaten Routes.LOGIN'e yönlendirecek.
        Get.snackbar(
          'Başarılı',
          'Başarıyla çıkış yapıldı.',
          backgroundColor: Colors.green,
          colorText: Colors.white,
          snackPosition: SnackPosition.BOTTOM,
          duration: const Duration(seconds: 2),
        );
      } catch (e) {
        print('ProfilScreen: Error during logout: $e');
        Get.snackbar(
          'Hata',
          'Çıkış yapılırken bir hata oluştu: $e',
          backgroundColor: Colors.red,
          colorText: Colors.white,
          snackPosition: SnackPosition.BOTTOM,
        );
      }
    }
  }

  // Profil düzenleme ekranına gitme metodu
  void goToProfileEdit() {
    // Mevcut kullanıcı verilerini argüman olarak gönder
    Get.toNamed(Routes.PROFILE_EDIT, arguments: userData.value)?.then((result) {
      // Düzenleme ekranından dönüldüğünde verileri yenile
      // Eğer düzenleme yapıldıysa ve ekran geri dönerken bir sonuç döndürülüyorsa
      // (örneğin true), o zaman _loadUserData() çağrılabilir.
      // Şu anki mantıkta her dönüşte veriyi yeniliyoruz.
      _loadUserData();
    });
  }

  // Bilgi kartı widget'ı
  Widget buildInfoCard(String label, String value, IconData icon) {
    return Card(
      elevation: 4,
      shadowColor: Colors.indigo.withOpacity(0.3),
      margin: const EdgeInsets.symmetric(vertical: 8),
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
      child: ListTile(
        leading: Icon(icon, color: Colors.indigo),
        title: Text(
          label,
          style: const TextStyle(
            fontWeight: FontWeight.w600,
            fontSize: 16,
            color: Colors.indigo,
          ),
        ),
        subtitle: Text(
          value.isNotEmpty ? value : '-',
          style: const TextStyle(fontSize: 15, color: Colors.black87),
        ),
      ),
    );
  }
}
